...
- Every few seconds (`VISION_SCAN_INTERVAL_SECONDS`, default 4), it opens the camera, grabs one frame, and immediately releases it — the camera isn't held open continuously, so the hardware "in use" light only flickers briefly per check rather than staying lit the whole time.
